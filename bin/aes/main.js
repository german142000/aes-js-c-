...
if(func == "-enc" && key.length == 16){
	let iv = [9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9];
	let strByteArray = [];
	for(let i = 0; i < plainText.length; i++) strByteArray.push(plainText.charCodeAt(i));
	var result = JSON.stringify(slowAES.encrypt(strByteArray, slowAES.modeOfOperation.OFB, key, iv));
	console.log(result);
} else if(func == "-dec" && key.length == 16){
	let iv = [9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9];
	let data = slowAES.decrypt(JSON.parse(plainText), slowAES.modeOfOperation.OFB, key, iv);
	let dataString = '';
	for(let i = 0; i < data.length; i++) dataString += String.fromCharCode(data[i]);
